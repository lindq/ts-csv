 ...
 * CSV writer.
 *
 * Writes arrays of data to CSV format. The flavor of CSV generated is
 * determined by an optional Dialect, with the default being the format
 * generated by Excel. Typical usage:
 *
 *   const data = [
 *     ['header 1', 'header 2'],
 *     ['foo', 'bar'],
 ...
 */
export function* writer(rows: Row[], opts: WriterOpts = {}) {
  const dialect = opts.dialect || excel;
...
/** Options for the recordWriter. */
interface RecordWriterOpts extends WriterOpts {
  /** Field names to specify the order of the columns in the output. */
  fields?: string[];
}
 ...
 */
export function*
    recordWriter(recordRows: RecordRow[], opts: RecordWriterOpts = {}) {
  const headers = opts.fields || Object.keys(recordRows[0] || {});
  const rows: Row[] = [
    headers,
    ...recordRows.map(row => headers.map(field => row[field] || '')),
  ];
  for (const row of writer(rows, {dialect: opts.dialect})) {
    yield row;
  }
}
